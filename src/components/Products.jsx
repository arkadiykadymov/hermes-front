...
export default class Products extends React.Component {
  ...
  loadProductsFromServer(page) {
    axios.get(api_url + '/products/' + this.props.category + '/' + page).then((response) => {
...
class PageNumber extends React.Component {
  ...
  render() {
    return (
      <div>
        {this.props.number === localStorage.getItem('currentPage') ? (
          <li className="page-item active">
            <a className="page-link" onClick={this.loadProductsFromServer}>
              {this.props.number}
            </a>
          </li>
        ) : (
...
class ProductsList extends React.Component {
  render() {
    ...
    return (
      <div>
        <ul key={products} className="card-columns">
          {products}
        </ul>
      </div>
    );
...
class Product extends React.Component {
  ...
  deleteProduct() {
    axios.get(api_url + '/products/delete/' + this.state.product.name).then((response) => {
  ...
  addToCart() {
    axios
      .post(
        'http://localhost:8000/api/v1/shoppingcart/addProduct',
        {
          ...
          <div className="card-body">
            ...
            <p className="card-text">
              <small className="text-muted">
                Storage counter - {this.props.product.storageCount}
              </small>
            </p>
            <button
              type="button"
            ...
            {localStorage.getItem('isAdmin') ? (
              <button type="button" className="btn btn-primary" onClick={this.deleteProduct}>
                Delete product
              </button>
            ) : (
              <div></div>
